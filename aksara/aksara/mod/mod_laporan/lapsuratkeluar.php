...
if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
	...
	switch($_GET[act]){
		...
		$t=mysql_query("select * from jns_sk where aktif='1' ");
		...
		<td>Alamat</td><td>Tanggal Surat</td><td>Aksi</td>
				</tr>
		";
		$no=1;
	...
	echo"<tr>
	<td>$no</td><td>";
	$qq=mysql_query("Select * from jns_sk where id='$r[id_jns]' and aktif='1'");
	...
	<td>$r[tglsrt]</td><td>
	<a href='mod/mod_surat/print.php?no=$r[nomor]&jns=$r[id_jns]'><abbr title='export ke word'><img src=img/icon.gif></abbr></a> ||
	<a href='?module=lapkeluar&act=delete&srt=$r[nomor]'><abbr title='hapus data'><img src=img/delete.png></abbr></a></td>
	...
	echo"<table id='example' class='table table-striped responsive-utilities jambo_table'>
		...
		if($jenis=='all'){
			 ...
			 $q=mysql_query("Select DISTINCT(nosrt) as nomor, id_jns ,tglsrt,nik from temp_srt where tglsrt >= '$tgl1' and tglsrt <= '$tgl2' and id_jns='$jenis'")or die(mysql_error());
		}
